  ...
  const transportContract = parsed._compactionRequest
    ...
    ? [
      ...
      ...(options?.delegateImplementation ? [
        ...
        "Delegation steps, in order: (1) Write a complete worker specification with OBJECTIVE, FILES AND OWNERSHIP, INTERFACES, CONSTRAINTS, VERIFICATION, and a request for a structured implementation report. (2) Call codex_tool_call with wire_name \"tool_search\" and arguments {\"query\": \"spawn_agent\"}, as a call of its own, and wait for its result. Tools it loads become callable only after that result returns, so never send the spawn in the same batch as the search. (3) Call codex_tool_call with wire_name \"multi_agent_v1__spawn_agent\" and arguments {\"agent_type\": \"sol_advisor_luna_implementer\", \"fork_context\": false, \"message\": <the specification>}. Pass no model or reasoning effort; the role already runs gpt-5.6-luna at max. (4) Call codex_tool_call with wire_name \"multi_agent_v1__wait_agent\" and timeout_ms exactly 30000, repeating until Luna finishes. (5) Inspect the actual diff and rerun the verification yourself.",
