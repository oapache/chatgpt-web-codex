...
import { ChatGptMemoryStore, contextPayloadMemoryEntries } from "./memory-store";
...
interface TurnChannel {
  ...
  /**
   * Full Codex context for this turn, staged once by the in-process owner (index.ts) instead of
   * being typed into the ChatGPT DOM as one or more staged messages. The MCP-side tool pulls it
   * over the already-open tunnel/broker channel, chunked to stay under one MCP response.
   */
  contextPayload?: string;
  /** Thread this turn belongs to, scoping every memory lookup it is allowed to make. */
...
export class TurnBroker implements TurnBrokerOwner {
  ...
  private memoryStore?: ChatGptMemoryStore;
  ...
  /**
   * Stage the full compiled Codex context for the MCP-side `context_fetch` pull instead of typing
   * it into the ChatGPT DOM. Called in-process by the owning adapter (index.ts) before the short
   * instruction message is sent, so the payload is already available the first time the model calls
   * the tool.
   */
  setContextPayload(token: string, payload: string): void {
    const channel = this.channels.get(token);
    if (!channel) throw new Error("turn token is invalid or expired");
    channel.contextPayload = payload;
  }

  /**
   * Bind this turn to a thread's durable memory and index the staged context into it. What
   * auto-compaction later drops from the live window stays reachable through codex_memory_search.
   */
  setMemoryScope(token: string, threadId: string, contextPayload?: string): void {
    const channel = this.channels.get(token);
    if (!channel) throw new Error("turn token is invalid or expired");
    if (!threadId.trim()) throw new Error("memory thread id is required");
    channel.memoryThreadId = threadId;
    if (contextPayload === undefined) return;
    const entries = contextPayloadMemoryEntries(contextPayload);
    if (entries.length > 0) this.memory().recordAll(threadId, entries);
  }

  recordMemory(token: string, kind: string, ref: string, text: string): void {
    const channel = this.channels.get(token);
    if (!channel?.memoryThreadId) return;
    this.memory().record(channel.memoryThreadId, kind, ref, text);
  }

  private memory(): ChatGptMemoryStore {
    this.memoryStore ??= new ChatGptMemoryStore();
    return this.memoryStore;
  }

  private memoryThread(token: string): string {
    const channel = this.channels.get(token);
    if (!channel) throw new Error("turn token is invalid or expired");
    if (!channel.memoryThreadId) throw new Error("this turn has no durable memory scope");
    return channel.memoryThreadId;
  }
  ...
  contextPayloadChunk(
    ...
    offset: number,
    limit: number,
  ): { chunk: string; totalChars: number; nextOffset: number | null } {
    const channel = this.channels.get(token);
    if (!channel) throw new Error("turn token is invalid or expired");
    if (channel.contextPayload === undefined) {
      throw new Error("Codex context is not staged for this turn yet");
    }
    const payload = channel.contextPayload;
    const chunk = payload.slice(offset, offset + limit);
    const nextOffset = offset + chunk.length < payload.length ? offset + chunk.length : null;
    return { chunk, totalChars: payload.length, nextOffset };
  }
  ...
  private async dispatch(request: BrokerRequest, socketSignal?: AbortSignal): Promise<unknown> {
    ...
    if (request.method === "context_fetch") {
      const token = request.token;
      if (typeof token !== "string" || token.length === 0) throw new Error("turn token is required");
      const offset = request.contextOffset ?? 0;
      const limit = request.contextLimit ?? MAX_BROKER_LINE_CHARS / 2;
      if (!Number.isSafeInteger(offset) || offset < 0) throw new Error("context fetch offset is invalid");
      if (!Number.isSafeInteger(limit) || limit <= 0 || limit > MAX_BROKER_LINE_CHARS / 2) {
        throw new Error("context fetch limit is invalid");
      }
      return this.contextPayloadChunk(token, offset, limit);
    }
    if (request.method === "memory_search") {
      const token = request.token;
      if (typeof token !== "string" || token.length === 0) throw new Error("turn token is required");
      if (typeof request.memoryQuery !== "string" || !request.memoryQuery.trim()) {
        throw new Error("memory query is required");
      }
      const limit = request.memoryLimit ?? 8;
      if (!Number.isSafeInteger(limit) || limit < 1 || limit > 50) throw new Error("memory limit is invalid");
      const threadId = this.memoryThread(token);
      return { hits: this.memory().search(threadId, request.memoryQuery, limit) };
    }
    if (request.method === "memory_get") {
      const token = request.token;
      if (typeof token !== "string" || token.length === 0) throw new Error("turn token is required");
      if (!Number.isSafeInteger(request.memoryId) || (request.memoryId as number) < 0) {
        throw new Error("memory id is invalid");
      }
      const threadId = this.memoryThread(token);
      const record = this.memory().get(threadId, request.memoryId as number);
      if (!record) throw new Error("memory record is not available for this turn");
      return { kind: record.kind, ref: record.ref, createdAt: record.createdAt, text: record.text };
    }
