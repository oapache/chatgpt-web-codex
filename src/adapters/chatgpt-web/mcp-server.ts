...
export async function runChatGptMcpServer(options: {
  ...
  if (contract === "native") {
    server.registerTool(
      "codex_context_fetch",
      {
        title: "Fetch the Codex task context",
        description: "Pull a chunk of the current turn's task context, staged by the local Codex adapter instead of being pasted into this chat. Call with offset 0 first, then keep calling with the returned next_offset until it is null, and concatenate every chunk in order to reconstruct the complete context JSON.",
        inputSchema: {
          turn_token: turnTokenSchema,
          offset: z.number().int().min(0).default(0),
          limit: z.number().int().min(1).max(1_000_000).default(200_000),
        },
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
      },
      async ({ turn_token, offset, limit }, extra) => {
        console.error(`[chatgpt-web-mcp] codex_context_fetch scope=${requestScopeSummary(extra)}`);
        const response = await callTurnBroker<{ chunk: string; totalChars: number; nextOffset: number | null }>(
          options.brokerSocketPath,
          { method: "context_fetch", token: turn_token, contextOffset: offset, contextLimit: limit },
          5_000,
          extra.signal,
        );
        return result({
          chunk: response.chunk,
          total_chars: response.totalChars,
  ...
  if (contract === "native") {
    server.registerTool(
      "codex_memory_search",
      {
        title: "Search this task's durable memory",
        description: "Keyword-search every message and tool result recorded for this Codex thread, including turns that context compaction already removed from the conversation. Returns ranked snippets with ids; pass an id to codex_memory_get for the full record. Use it whenever the task refers to earlier work that is no longer visible in the context.",
        inputSchema: {
          turn_token: turnTokenSchema,
      ...
      async ({ turn_token, query, limit }, extra) => {
        console.error(`[chatgpt-web-mcp] codex_memory_search scope=${requestScopeSummary(extra)}`);
        const response = await callTurnBroker<{ hits: unknown[] }>(
          options.brokerSocketPath,
          { method: "memory_search", token: turn_token, memoryQuery: query, memoryLimit: limit },
          5_000,
          extra.signal,
        );
        return result({ hits: response.hits, total: response.hits.length });
      },
    );
    ...
    server.registerTool(
      "codex_memory_get",
      {
        title: "Read one durable memory record",
        description: "Return the complete recorded text for an id returned by codex_memory_search.",
        inputSchema: {
          turn_token: turnTokenSchema,
      ...
      async ({ turn_token, id }, extra) => {
        console.error(`[chatgpt-web-mcp] codex_memory_get scope=${requestScopeSummary(extra)}`);
        const response = await callTurnBroker<{
          ...
          options.brokerSocketPath,
          { method: "memory_get", token: turn_token, memoryId: id },
          5_000,
          extra.signal,
        );
        return result({
