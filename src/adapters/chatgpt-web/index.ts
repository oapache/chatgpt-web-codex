...
export function createChatGptWebAdapter(
  ...
  /**
   * DEV-only experimental transport: instead of typing one or more staged JSON context messages
   * into the ChatGPT DOM (Bigger Context), send a short instruction and let ChatGPT pull the same
   * context itself through the codex_context_fetch MCP tool over the existing tunnel/broker
   * channel. Requires the in-process TurnBroker (native contract, local tools attached).
   */
  const experimentalContextViaMcp = process.env.CODEX_CHATGPT_WEB_CONTEXT_VIA_MCP === "1";
  const delegateImplementation = process.env.CODEX_CHATGPT_WEB_DELEGATE_IMPLEMENTATION === "1";
  ...
  const startRuntime = (
    ...
    const releaseRetainedConversation = conversationKey && retainedLauncherDescriptor
      ...
      : undefined;
    const compileOptionsFor = (input: CodexParsedRequest, turnToken?: string) => {
      if (manualRequest) return {};
      // Context-via-MCP replaces Bigger Context's DOM staging outright when both are enabled:
      // computing experimentalMultipartParts here would just be discarded work.
      const contextFetchToken = experimentalContextViaMcp
        && structuredBroker
        && turnToken !== undefined
        ? turnToken
        : undefined;
      const experimentalMultipartParts = experimentalBiggerContext && contextFetchToken === undefined
      ...
      return {
        ...
        ...(experimentalMultipartParts !== undefined
          ...
          : {}),
        ...(contextFetchToken !== undefined ? { contextFetchToken } : {}),
        ...(delegateImplementation ? { delegateImplementation: true as const } : {}),
      };
    };
    ...
    const prepareWith = async (input: CodexParsedRequest) => {
      ...
      try {
        const compiled = compileChatGptWebPrompt(
          ...
          turnCapabilities,
          turnToken,
          compileOptionsFor(input, turnToken),
        );
        if (compiled.contextPayload !== undefined) {
          // structuredBroker is guaranteed here: compileOptionsFor only sets contextFetchToken
          // when structuredBroker is present.
          structuredBroker!.setContextPayload(turnToken, compiled.contextPayload);
        }
        if (structuredBroker && identity.threadId) {
          structuredBroker.setMemoryScope(turnToken, identity.threadId, compiled.indexableContext);
        }
