...
export function estimateCompiledChatGptWebInputTokens(
  ...
  const acknowledgementTokens = compiled.multipart
    ...
    : 0;
  // Context fetched over MCP never appears in a composer message, but the model still ingests it
  // through codex_context_fetch. Counting it keeps Codex's own context accounting and its
  // auto-compaction trigger honest; only the per-message budget excludes it.
  const contextFetchTokens = compiled.contextPayload === undefined
    ? 0
    : estimateTokens(compiled.contextPayload, modelId);
